...
def cw(model: nn.Module, images: torch.Tensor, labels: torch.Tensor, c: float = 1.0, kappa: int = 1000,
       steps: int = 1000, lr: float = 0.01) -> torch.Tensor:
    """
    Craft untargeted C&W L2 adversarial examples.

    Optimises a tanh-space perturbation to minimise: ||adv-clean||_2^2 + c*f(adv) , where f = the margin between the
    true class logits and the best other logits, floored at -kappa. The tanh reparametrisation keeps every iterate in
    [0, 1] by construction, so no clipping is needed. For each sample the lowest distortion perturbation that actually
    flips the prediction is returned. Samples that never flip returned unchanged. The attack is deterministic given the
    model and input, so no seeding is required.

    The model must be in eval mode: the thousand forward passes would otherwise overwrite BatchNorm running statistics
    with adversarial batches. Its parameters' requires_grad flags are saved and restored, and no parameter gradients
    are left populated, so the model is unchanged on return.

    Gradients are computed under an explicit torch.enable_grad context so the call is safe inside a torch.no_grad
    evaluation loop. It's not safe inside torch.inference_mode which forbids requires_grad on its tensors.
    ...
    Args:
        ...
        steps: Adam iteration, must be >= 1.
    ...
    Raises:
        ValueError: if c or kappa <0, steps <1 or lr <= 0.
    """
    if c < 0:
    ...
    # tanh inverted so the first iterate == clean input
    ...
    try:
        with torch.enable_grad():
            for _ in range(steps):
                ...
                dist = (adv - clean).flatten.pow(2).sum(dim=1)
