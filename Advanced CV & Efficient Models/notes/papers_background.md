...
architecture that directly optimises accuracy under real hardware latency constraints on mobile devices?

The key tension: accuracy vs latency on mobile CPUs, measured in **actual wall-clock
milliseconds**, not just FLOPs.
...
- Iteratively proposes small changes (e.g., reduce filters, remove a layer) that respect a latency budget, then briefly retrains to measure accuracy impact.  
- Complements NAS by tuning widths without re-running the full search.

Together, NAS chooses the global structure while NetAdapt refines local widths under latency targets, a combination neither algorithm achieves alone.
...
h-swish is easy to quantise, faster on fixed-point hardware, and is applied mainly in deeper layers where channel counts are high so the per-op cost is amortised. Swish/h-swish improves gradient flow relative to ReLU and is related to GELU used in Transformers.

#### 3. Squeeze-and-Excitation (SE) Blocks

MobileNetV3 borrows SE blocks from SENet and selectively inserts them into certain bottleneck blocks:

```text
SE block: Global Average Pool → FC → ReLU → FC → h-sigmoid → scale channels
```

Each channel receives a learned scalar gate from global context, allowing the network to recalibrate channel importance at modest parameter cost. MobileNetV3 applies SE in the expansion (high-channel) phase of bottlenecks to keep latency low.

#### 4. Redesigned Head and Stem

- **Stem:** The first convolution (16 filters, stride 2) is retained, but early layers are thinned using NetAdapt; MobileNetV2’s stem is shown to be over-parameterised relative to its latency contribution.  
- **Head:** MobileNetV2 uses a 1×1 conv → global average pool → 1×1 conv sequence. MobileNetV3 moves the expensive 1×1 conv **after** pooling, so it operates on 1×1 rather than 7×7 feature maps, saving latency with negligible accuracy loss.
...
For dense prediction tasks, the authors propose **Lite Reduced Atrous Spatial Pyramid Pooling (LR-ASPP)**:

- Replaces the full ASPP of DeepLabV3+ with a lightweight alternative.  
- Uses a single global average pooling branch + 1×1 conv instead of multiple atrous rates.  
- Achieves about 30% lower latency than MobileNetV2’s R-ASPP at similar Cityscapes accuracy.

---

### Key Results

Representative ImageNet and downstream numbers from the paper:
...
- COCO detection: MobileNetV3-Large is reported as over 25% faster than MobileNetV2 at comparable mAP when using channel reduction.  
- Cityscapes segmentation: LR-ASPP is about 30% faster than MobileNetV2’s R-ASPP at similar accuracy.
...
- NAS and NetAdapt search are computationally expensive (TPU time); practitioners typically use the released MobileNetV3 checkpoints rather than re-running search.  
- The architecture is **hardware-specific**: latency gains are measured on ARM CPUs; benefits may not transfer directly to GPUs or DSPs without re-searching on the target platform.  
- SE blocks improve accuracy but add memory bandwidth overhead, which can partially offset latency savings on some hardware.
...
- When targeting mobile deployment, optimise for **measured latency**, not just FLOPs.
...
Prior detectors (R-CNN, DPM, and later Fast R-CNN) decomposed detection into separate stages: region proposal → feature extraction → classification → bounding box regression. These stages were trained separately and were relatively slow; R-CNN required tens of seconds per image, and even Fast R-CNN with Selective Search achieved only around 0.5 fps on VOC.

**YOLO’s insight:** Frame object detection as a **single regression problem** over the image: one network, one pass, one loss, trained end-to-end. Hence “You Only Look Once.”
...
The backbone is a custom CNN inspired by GoogLeNet. It is first pretrained on ImageNet classification at 224×224, then fine-tuned for detection at 448×448 to improve localisation of small objects.

---

### Core Detection Mechanism

**Grid cell responsibility:** Each grid cell is responsible for detecting objects whose **centre** falls inside that cell.
...
YOLOv1 uses a hand-crafted sum of squared errors (SSE) with task-specific weighting. A concise form (using indices i for cells, j for boxes, c for classes):
...
- Real-time speed: base YOLO runs at ~45 fps; Fast YOLO runs at ~155 fps on a Titan X (as reported in the paper).
...
Representative results from Redmon et al. and related literature:
...
YOLO trades roughly 6–7 mAP points vs. Fast R-CNN on VOC 2007 for two orders of magnitude higher throughput (0.5 fps → 45 fps), making it the first widely adopted real-time detector with competitive accuracy.
...
- Understanding YOLOv1’s struggles with small, dense objects motivates anchor-based detectors (YOLOv2+), feature pyramids, and eventually anchor-free and transformer-based detectors.
...
Before DETR, major detectors relied on hand-engineered components:
...
These components require manual design and prevent fully end-to-end set prediction. DETR asks whether transformers can eliminate such heuristics by learning object detection as a **set prediction** problem with global self-attention.
...
DETR uses N=100 learned embeddings, called **object queries**, fed to the transformer decoder as the target sequence.
...
- Queries have no explicit spatial prior; their behaviour emerges from training, removing the need for hand-crafted anchors.

#### 2. Bipartite Matching Loss

Given N predictions and M ground-truth objects (M ≤ N):

1. Compute a cost matrix between predictions and ground-truth boxes: classification cost plus box cost (e.g., L1 + GIoU).  
2. Use the **Hungarian algorithm** to find a one-to-one matching between predictions and ground truth (bipartite matching).  
3. Compute loss only on matched pairs; unmatched predictions are trained as “no object.”

This one-to-one supervision removes the need for NMS; duplicate predictions of the same object are penalised via the matching.

**Box loss:** combines L1 on normalised coordinates and Generalised IoU (GIoU) loss, weighted by λ terms.

#### 3. Transformer Encoder

The encoder applies multi-head self-attention over the flattened feature map, allowing each location to attend to all others and aggregating global context; this replaces hand-designed context modules and multi-stage feature aggregation.

#### 4. Transformer Decoder (Cross-Attention)

Each object query attends to encoder outputs via cross-attention, then interacts with other queries via self-attention. This lets the model capture object–object relations and resolve which queries take responsibility for which objects.

---

### Panoptic Segmentation Extension

DETR extends to panoptic segmentation by adding a simple pixel-wise FFN over upsampled decoder attention maps. For each detected object, corresponding attention maps are combined to form a mask, without designing a new mask head.

---

### Results (COCO 2017 val)

Representative results (DETR-R50, R101) as reported in the original and follow-up implementations:
...
- **Slow convergence:** Original DETR requires ~500 training epochs on COCO to reach its best AP, vs ~36 epochs for the 3× Faster R-CNN schedule. Matching-based supervision yields relatively weak gradients early on when predictions are random.  
- **Small object performance:** The backbone’s 32× downsampling and lack of explicit multi-scale features hurt AP_S; Deformable DETR and later variants add multi-scale and deformable attention to address this.  
- **Quadratic attention cost:** Standard MHSA is O(n²) in sequence length, making the encoder relatively expensive on high-resolution feature maps.  
- **Fixed slots (N=100):** DETR cannot predict more than N objects per image; this is usually sufficient for COCO but is conceptually a capacity limit.

These limitations directly motivated Deformable DETR, Conditional/Anchor-based DETRs, DN-DETR and DINO (detector), which progressively improve convergence and small-object performance.
...
DETR’s ideas underpin many modern open-vocabulary and grounded detection models (e.g., Grounding DINO, OWL-ViT), which combine language-conditioning with DETR-style set prediction.
...
- **Object queries as learned slots** provide a general mechanism for variable-cardinality outputs without NMS.  
...
Around 2016, deployment on mobile/embedded hardware was constrained by:

- **Memory:** Models like AlexNet (~240 MB) and VGG-16 (~552 MB) exceeded on-chip SRAM capacities; off-chip DRAM access is roughly two orders of magnitude more energy-expensive than on-chip SRAM.  
- **Compute:** Embedded CPUs/GPUs had limited FP32 throughput.

Goal: compress networks so they fit entirely in on-chip SRAM, enabling faster and more energy-efficient inference with minimal or no accuracy loss.
...
Each stage enables the next to be more effective: pruning exposes sparsity, quantization reduces bit-width, and Huffman coding exploits non-uniform symbol distributions.
...
Weights for surviving connections remain full precision; only connectivity changes. Sparse weights are stored in a **Compressed Sparse Row (CSR)**-like format with relative indices to reduce index storage.

**Compression from pruning alone (paper values):**
...
Most pruned parameters are in fully connected layers; convolutional layers are pruned more conservatively to preserve accuracy.

---

### Stage 2: Trained Quantization (Weight Sharing)

After pruning, each remaining weight is still stored as a 32-bit float. Stage 2 reduces precision by **weight sharing** via k-means clustering.

**Mechanism – k-means over weights:**  

1. For each layer, cluster its weights into k clusters (e.g., k=256 → 8-bit indices).  
2. Replace each weight by an index into the centroid table.  
3. Retrain only the centroids: gradients from all weights in a cluster are aggregated and applied to that centroid.
...
Typical precision in the paper: 8 bits for convolutional layers, 4–5 bits for fully connected layers (more tolerant to quantisation).

---

### Stage 3: Huffman Coding

After weight sharing, cluster indices follow a skewed distribution (many weights near zero). Huffman coding assigns shorter codes to frequent indices, giving an additional compression factor.

Sparse CSR indices are also Huffman-coded. This yields an extra ~20–30% reduction on top of pruning + quantization, depending on layer.
...
Both models then fit into 8–16 MB SRAM budgets typical of embedded systems, avoiding DRAM access.
...
- Coding compresses the skewed symbol distribution.

---

### Limitations

- **Irregular sparsity:** Unstructured connection pruning yields sparse matrices that are hard to accelerate efficiently on standard GPUs (which prefer dense GEMMs). Real speedups require sparse kernels or specialised hardware.  
- **Structured pruning alternatives:** Later work explores filter/channel pruning to produce dense but smaller models that accelerate on commodity GPUs, at the cost of somewhat lower compression ratios for the same accuracy.  
- **Retraining cost:** Each stage requires fine-tuning, which adds training overhead.  
- **Outdated quantisation method:** Modern **quantisation-aware training (QAT)** and post-training quantisation (PTQ) methods largely supersede k-means weight sharing; frameworks like PyTorch and TensorRT provide QAT primitives natively.
...
- The core insight—that most weights are redundant and can be compressed with little or no accuracy loss—carries directly into LLM compression (INT4, INT8, GGUF, AWQ, etc.).
...
These four papers collectively cover: how to detect objects efficiently (YOLO), how to detect them without hand-crafted heuristics (DETR), how to build mobile-efficient backbones (MobileNetV3), and how to compress networks aggressively for deployment (Deep Compression).
