...
**Semantic segmentation** assigns a class label to every pixel in the image. All pixels belonging to the same category receive the same label, regardless of whether they belong to different object instances. For example, a street scene would have all car pixels labeled “car”, all pedestrians labeled “person”, etc., with no distinction between individual cars.

**Instance segmentation** detects each individual object and produces a pixel-precise mask for every instance. In the same street scene, each car and each pedestrian would have its own mask (car 1, car 2, pedestrian 1, pedestrian 2, …). Instance segmentation combines object detection (which object, where) with segmentation (which pixels).

Semantic segmentation is sufficient when only class regions matter (e.g., road vs sidewalk vs sky). Instance segmentation is required when objects must be counted, tracked, or analysed individually (e.g., cell counting in microscopy, vehicle or pedestrian tracking in autonomous driving).

---

## U-Net (Ronneberger, Fischer, Brox — MICCAI 2015)

**Reference:** Ronneberger O., Fischer P., Brox T. “U-Net: Convolutional Networks for Biomedical Image Segmentation.” MICCAI 2015, LNCS 9351.

### Motivation

Before U-Net, a common approach to pixel-wise prediction was sliding-window classification: a patch around each pixel is fed through a CNN to predict that pixel’s label. This is:

- Slow: the network runs once per pixel, with heavy overlap between neighbouring patches.  
- Limited by a trade-off: larger patches offer more context but blur localisation; smaller patches sharpen localisation but lose global context.

U-Net resolves this trade-off using an encoder–decoder architecture with skip connections that provide both global context and fine spatial detail.
...
Each 3×3 valid conv shrinks spatial dimensions by 2 (1 pixel per side). The number of feature channels doubles at each downsampling step.
...
- Two 3×3 convs with ReLU.

**Final layer.** A 1×1 convolution maps the last 64-channel feature map to the desired number of classes (e.g., 2 for foreground/background).

**Standard configuration channel and size progression (original paper):**
...
The output (388×388) is smaller than the input (572×572) due to repeated valid convolutions. The paper handles larger images with an “overlap-tile” strategy: process overlapping tiles and stitch predictions.

### ASCII Architecture Diagram

The diagram below follows the original U-Net structure (valid convs, cropping for skips):
...
- **ResNet skip connections:**  
  Add input to output (element‑wise) to form residual blocks, primarily to combat vanishing gradients and ease optimisation. They do not change the spatial resolution hierarchy; skips are within the same resolution level.

- **U-Net skip connections:**  
  Concatenate encoder and decoder feature maps at corresponding resolutions to restore fine-grained spatial information lost through pooling and valid convolutions. This directly addresses an information bottleneck, not gradient flow.

In short: ResNet skips are about **optimisation**; U‑Net skips are about **restoring spatial detail**.

### Loss Function

The original U-Net uses pixel-wise softmax followed by cross-entropy, weighted by a spatial weight map:

```text
w(x) = w_class(x) + w0 * exp( - (d1(x) + d2(x))^2 / (2 * sigma^2) )
```

- `w_class(x)`: class-balance term (higher for rare classes).
- `d1(x)`: distance from pixel x to the nearest cell border.
- `d2(x)`: distance to the second nearest cell border.
- Recommended values: w0 = 10, sigma = 5 pixels (paper defaults).
...
U-Net was designed for biomedical segmentation with limited annotated data. Two key factors:
...
- Aggressive **elastic deformation augmentation**: random smooth displacement fields applied to training images/masks simulate plausible biological deformations, effectively enlarging the training set and improving robustness.

---

## Mask R-CNN (He, Gkioxari, Dollár, Girshick — ICCV 2017)

**Reference:** He K., Gkioxari G., Dollár P., Girshick R. “Mask R-CNN.” ICCV 2017.

### Problem Statement

Mask R-CNN addresses **instance segmentation**: detect each object instance and produce a binary mask for each one. It extends Faster R-CNN—originally a two-stage object detector predicting classes and bounding boxes—by adding a parallel mask prediction branch.

### Architecture Overview

Mask R-CNN has three key stages:

1. **Backbone + FPN.**  
   - A ResNet-50/101 backbone extracts feature maps.  
   - A Feature Pyramid Network (FPN) builds a multi-scale feature pyramid, combining high-resolution and high-semantic-level features to handle objects of different sizes.

2. **Region Proposal Network (RPN).**  
   - Same concept as in Faster R-CNN: slides over FPN maps, proposes candidate boxes (anchors refined into proposals) with objectness scores; low-score proposals are filtered via NMS.

3. **Per-RoI heads (parallel branches).**  
   For each proposal, RoIAlign extracts fixed-size features from the appropriate FPN level, then three heads operate in parallel:  
...
RoIPool (used in Fast/Faster R-CNN) quantises RoI coordinates and bin boundaries to integers on the feature map grid. This introduces misalignment between the proposal and pooled features. While acceptable for bounding box detection, this misalignment hurts pixel-level mask prediction.

**RoIAlign.**  

RoIAlign removes rounding:
...
No coordinate rounding is performed. This significantly improves mask AP, especially at higher IoU thresholds.
...
The mask branch operates on a higher‑resolution RoI feature map than the box/class heads:
...
A crucial design choice: **decouple mask and class prediction.**
...
This decoupling improves mask AP compared to a joint class+mask softmax, as shown in the paper (+2.1 mask AP in their ablation).
...
At inference: choose the mask for the predicted class, rescale it to the RoI box inside the original image, then threshold (e.g. at 0.5) to produce a binary mask.

### Comparison with Semantic Segmentation

Mask R-CNN is an instance segmentation model. It differs from semantic segmentation architectures like U-Net in both task and structure:
...
where A is the predicted region and B is the ground-truth. It ranges from 0 (no overlap) to 1 (perfect overlap). This is the standard localisation metric for segmentation and detection (for boxes, IoU is computed on bounding boxes).

**Dice coefficient.**

Used extensively in medical segmentation:

```text
Dice = 2 * |A ∩ B| / (|A| + |B|)
```

Dice and IoU are monotonically related: Dice = 2 * IoU / (1 + IoU). Dice can be more sensitive for small structures because the union denominator in IoU grows faster with false positives.

**Mask AP (COCO).**

Mask R-CNN is evaluated on COCO using mask average precision (AP):

- AP is computed over multiple IoU thresholds (0.5:0.05:0.95) using mask IoU, not box IoU.
- Commonly reported metrics: AP, AP_50 (IoU >= 0.5), AP_75, AP_S / AP_M / AP_L (small/medium/large objects).

This mirrors detection AP but uses mask overlap instead of bounding boxes.
