...
The standard approach before EfficientNet was to scale only one dimension -
...
The three base coefficients (1.20, 1.10, 1.15) were found via neural architecture
search on a small baseline (EfficientNet-B0, φ=0). B1 through B7 apply φ=1 through 7
to the **same architecture**, producing a family of models that scales smoothly along
the accuracy-efficiency frontier. EfficientNet-B0 (φ=0) has 4.0M parameters;
EfficientNet-B7 (φ=7) has 66M — the same structural blueprint, scaled by compound φ.

The benchmark result supports this directly: EfficientNet-B0 achieves 90.06% with
only 4.0M parameters, the best accuracy-per-parameter ratio (22.5) in the group,
demonstrating that balanced scaling extracts more value per parameter than the
...
Crucially: nearly half of the total +5.7pp gain came from the training recipe alone
(+2.7pp), with zero architectural changes. The paper's deepest finding is that 
the ConvNet vs Transformer debate is partly a training recipe debate.
...
For context:
- ViT-Tiny achieves 14.7 with 5.5M parameters - better ratio than ResNet-18 but
  3pp lower accuracy than EfficientNet-B0 with similar parameter count.
...
at 90% accuracy from 4M parameters. This is a direct demonstration of φ working as
intended — balanced scaling extracts more representational capacity per parameter
than any single-dimension scaling strategy.
...
comparison is a separate experiment (Week 2 training runs on Kaggle T4).
