...
### 2. Encoder f(·)
Any backbone (ResNet-50 in the original). Outputs representation `h`.  This is what you keep and fine-tune downstream.
...
### 4. Normalized Temperature-scaled Cross Entropy (NT-Xent Loss)
...
- `τ` - temperature hyperparameter (paper reports 0.1 as optimal)
- Denominator sums over all 2N−1 other views
- Final loss averages over both directions (i→j and j→i)

Softmax cross-entropy where the "correct class" is the positive pair. Temperature controls sharpness, meaning lower τ = harder negatives = harder problem.
...
| **BYOL** | 2020 | ❌ | ✅ | No negatives; predictor + EMA target prevents collapse |
