...
def hysteresis(image: np.ndarray, strong, weak) -> np.ndarray:
    ...
    while True:
        prev = connected.copy()
        padded = np.pad(
            array=connected, pad_width=1, mode="constant", constant_values=False
        )
...
def double_threshold(
    image: np.ndarray, low_ratio=0.05, high_ratio=0.15
) -> tuple[ndarray[Any, dtype[Any]], int, int]:
    ...
    conditions = [
        ...
        (high_threshold > image) & (image >= low_threshold),
    ]
    choices = [strong, weak]
...
def non_max_suppression(magnitude: np.ndarray, direction: np.ndarray) -> np.ndarray:
    ...
    directions = [
        (
            ...
            [(0, 1), (0, -1)],
        ),  # 0 degrees, neighbors right and left
        (
            (angle >= 22.5) & (angle < 67.5),
            [(-1, 1), (1, -1)],
        ),  # 45 degrees, neighbors top-right and bottom-left
        (
            (angle >= 67.5) & (angle < 112.5),
            [(-1, 0), (1, 0)],
        ),  # 90 degrees, neighbors top and bottom
        (
            (angle >= 112.5) & (angle < 157.5),
            [(-1, -1), (1, 1)],
        ),  # 135 degrees, neighbors top-left and bottom-right
    ]
    ...
    for directed_pixel, [(dx1, dy1), (dx2, dy2)] in directions:
        ...
        local_max = (magnitude[1:-1, 1:-1] >= neighbor1) & (
            magnitude[1:-1, 1:-1] >= neighbor2
        )
        suppressed[1:-1, 1:-1][interior_directed_pixel & local_max] = magnitude[
            1:-1, 1:-1
        ][interior_directed_pixel & local_max]

    return suppressed


def canny_edge_detector(
    image: np.ndarray, low_ratio=0.05, high_ratio=0.15, sigma=1.4
) -> np.ndarray:
    """Args:
    ...
    thresholded, strong, weak = double_threshold(
        image=nms_image, low_ratio=low_ratio, high_ratio=high_ratio
    )
