...
All box coordiantes are in absolute feature-map space (pre scaled).
"""

import torch

def bilinear_interpolation(feature_map: torch.Tensor, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
    """
    Sample a feature map at continuous (x, y) coordinates using bilinear interpolation.
    Coordinates outside the feature map boundaries are clampled to the nearest valid pixed before interpolation,
    matching the boundary behaviour used in the Mask R-CNN reference implementation.

    Args:
        feature_map: Feature map of shape (C, H, W).
        x: Floating point row coordinates, shape (N,).
        y: Floating point column coordinates, shape (N,).
    ...
    x = x.clamp(min=0, max=float(H-1))
    y = y.clamp(min=0, max=float(W-1))
    ...
    w_x1 = (x - x0.float()).unsqueeze(0)  # (1, N)
    w_x0 = 1.0 - w_x1
    w_y1 = (y - y0.float()).unsqueeze(0)  # (1, N)
    w_y0 = 1.0 - w_y1
...
def roi_align(feature_map: torch.Tensor, boxes: torch.Tensor, output_size: int,
    ...
    """
    Extract fixed-sized features for a set or RoI boxes using RoI Align.
    ...
    No coordiante quantization is applied, matching He et al., ICCV 2017.
